...
using System.Text;
using System.Xml;
using RecyclableMemory = OfficeOpenXml.Utils.RecyclableMemoryStream;
...
namespace Magicodes.IE.EPPlus
{
    internal class ImageReader
    {
        ...
        internal static ExcelImageFormat? GetPictureType(MemoryStream ms)
        {
            ...
            {
                return ExcelImageFormat.Tif;
            }
            else if (IsIco(br))
            {
                return ExcelImageFormat.Ico;
            }
            else if (IsWebP(br))
            {
                return ExcelImageFormat.Webp;
            }
            else if (IsEmf(br))
            {
            ...
            {
                return ExcelImageFormat.Svg;
            }
            else if (IsGZip(br))
            {
                ms.Position = 0;
                var img = ExtractImage(ms.ToArray(), out ExcelImageFormat? pt);
                return pt;
            }
            return null;
        }
        ...
        internal static bool TryGetImageBounds(ExcelImageFormat pictureType, MemoryStream ms, ref double width, ref double height, out double horizontalResolution, out double verticalResolution)
        {
            width = 0;
            height = 0;
            horizontalResolution = verticalResolution = STANDARD_DPI;
            try
            {
                ms.Seek(0, SeekOrigin.Begin);
                if (pictureType == ExcelImageFormat.Bmp && IsBmp(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                if (pictureType == ExcelImageFormat.Jpeg && IsJpg(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                if (pictureType == ExcelImageFormat.Gif && IsGif(ms, ref width, ref height))
                {
                    return true;
                }
                if (pictureType == ExcelImageFormat.Png && IsPng(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                if (pictureType == ExcelImageFormat.Emf && IsEmf(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                if (pictureType == ExcelImageFormat.Wmf && IsWmf(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                else if (pictureType == ExcelImageFormat.Svg && IsSvg(ms, ref width, ref height))
                {
                    return true;
                }
                else if (pictureType == ExcelImageFormat.Tif && IsTif(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                else if (pictureType == ExcelImageFormat.Webp && IsWebP(ms, ref width, ref height, ref horizontalResolution, ref verticalResolution))
                {
                    return true;
                }
                else if (pictureType == ExcelImageFormat.Ico && IsIcon(ms, ref width, ref height))
                {
                    return true;
                }
                return false;
        ...
        internal static byte[] ExtractImage(byte[] img, out ExcelImageFormat? type)
        {
            if (IsGZip(img))
            {
                try
                {
                    ...
                    if (IsEmf(br))
                    {
                        type = ExcelImageFormat.Emf;
                    }
                    else if (IsWmf(br))
                    {
                        type = ExcelImageFormat.Wmf;
                    }
                    else
                    {
                        type = null;
        ...
        {
            width = height = 0;
            horizontalResolution = verticalResolution = STANDARD_DPI * (1 + 1 / 3); //Excel seems to render webp at 1 1/3 size.
            using (var br = new BinaryReader(ms))
            {
                if (IsWebP(br))
                {
            ...
            using (var br = new BinaryReader(ms))
            {
                if (IsEmf(br))
                {
                    ...
                    if (signature.Trim() == "EMF")
                    {
                        ...
                        horizontalResolution = width / ((frame[2] - frame[0]) * HUNDREDTH_TH_MM_TO_INCH * STANDARD_DPI) * STANDARD_DPI;
                        verticalResolution = height / ((frame[3] - frame[1]) * HUNDREDTH_TH_MM_TO_INCH * STANDARD_DPI) * STANDARD_DPI;
